...
from mt_dnn.shared_model import MultiClsMTDNNModel
import copy
...
def train_config(parser):
    ...
    # CAMERO args
    parser.add_argument('--n_models', default=2, type=int, help="number of models in the collaborative learning.")
    parser.add_argument('--teaching_type', default="ensemble", type=str, help="type of teaching logits. choices: [ensemble, pairwise]")
    parser.add_argument('--pert_type', default="dropout", type=str, help="type of perturbations. choices: [none, dropout, adv, r3f]")
    parser.add_argument('--eval_ensemble', action='store_true', help="evaluate the ensembled model.")
    parser.add_argument('--kd_alpha', default=1.0, type=float, help="the weight of distillation loss.")

    # adv perturbation args for CAMERO
...
def evaluation(model, datasets, data_list, task_defs, output_dir='checkpoints', epoch=0, n_updates=-1, with_label=False, tensorboard=None, glue_format_on=False, test_on=False, device=None, logger=None, model_fix="", group_id=None):
    # eval on rank 1
    print_message(logger, "Evaluation")
    if test_on:
        test_prefix=model_fix + " Test"
    else:
        test_prefix=model_fix + " Dev"
    ...
    return test_prediction_list, test_gold_list
...
def initialize_model(init_model, encoder_type, opt, args, device, num_all_batches, silent_mode=False):
    ...
    model = MultiClsMTDNNModel(opt, device=device, state_dict=state_dict, num_train_step=num_all_batches)
    ...
    if args.encode_mode:
        for idx, dataset in enumerate(args.test_datasets):
            ...
            torch.save(encoding, os.path.join(output_dir, '{}_{}_encoding.pt'.format(mode, dataset)))
...
def main():
    ...
    train_collater = Collater(dropout_w=args.dropout_w, encoder_type=encoder_type, soft_label=False, max_seq_len=args.max_seq_len, do_padding=args.do_padding)
    train_datasets = []
    train_data_list = []
    ...
    ##################################
    model = initialize_model(args.init_checkpoint, encoder_type, opt, args, device, num_all_batches)
    ###################################
    ...
    for epoch in range(0, args.epochs):
        ...
        for i, (batch_meta, batch_data) in enumerate(multi_task_train_data):
            ...
            if batch_data[0].shape[0] < args.batch_size:
                continue
            ...
            if (model.updates) % (args.log_per_updates) == 0 or model.updates == 1:
                ...
                print_message(logger, 'Task [{0:2}] updates[{1:6}] train loss[{2:.5f}]{3} remaining[{4}]'.format(task_id,
                                                                                                    ...
                                                                                                    ramaining_time))
                if args.tensorboard:
                    tensorboard.add_scalar('model/loss', model.train_loss.avg, global_step=model.updates)

        dev_predictions, test_predictions = [None for _ in range(args.n_models)], [None for _ in range(args.n_models)]
        for j in range(args.n_models):
            dev_predictions[j], dev_gold = evaluation(model, args.test_datasets, dev_data_list, task_defs, output_dir, epoch, with_label=True, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=False, device=device, logger=logger, model_fix="m"+str(j), group_id=j)
            test_predictions[j], test_gold = evaluation(model, args.test_datasets, test_data_list, task_defs, output_dir, epoch, with_label=False, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=True, device=device, logger=logger, model_fix="m"+str(j), group_id=j)
            print_message(logger, '[new model {} test scores at {} saved.]'.format(j, epoch))

        if args.eval_ensemble:
            evaluation(model, args.test_datasets, dev_data_list, task_defs, output_dir, epoch, with_label=True, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=False, device=device, logger=logger, model_fix="logits_ensemble", group_id=[j for j in range(args.n_models)])
            if len(test_predictions[0]) != 0:
                evaluation(model, args.test_datasets, test_data_list, task_defs, output_dir, epoch, with_label=False, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=True, device=device, logger=logger, model_fix="logits_ensemble", group_id=[j for j in range(args.n_models)])
            print_message(logger, '[new ensemble scores at {} saved.]'.format(epoch))

        if args.local_rank in [-1, 0]:
            m_file = os.path.join(output_dir, 'model_{}.pt'.format(epoch))
            model.save(m_file)
