...
class MultiClsSANBertNetwork(nn.Module):
    def __init__(self, opt, bert_config=None, initial_from_local=False):
        super(MultiClsSANBertNetwork, self).__init__()
        self.dropout_list = nn.ModuleList([nn.ModuleList() for _ in range(opt["n_models"])])
        ...
        self.scoring_list = nn.ModuleList([nn.ModuleList() for _ in range(opt["n_models"])])
        self.pooler_list = nn.ModuleList([])
        for j in range(opt["n_models"]):
            for task_id in range(len(task_def_list)):
                ...
                self.dropout_list[j].append(dropout)
                task_obj = tasks.get_task_obj(task_def)
                if task_obj is not None:
                    # Move this to task_obj
                    self.pooler_list.append(Pooler(hidden_size, dropout_p=opt['dropout_p'], actf=opt['pooler_actf']))
                    ...
                    if decoder_opt == 1:
                        ...
                        out_proj = nn.Linear(hidden_size, lab)
                self.scoring_list[j].append(out_proj)
        self.config = opt

    def embed_encode(self, input_ids, token_type_ids=None, attention_mask=None):
        ''' copied from https://github.com/namisan/mt-dnn/blob/master/mt_dnn/matcher.py'''
    ...
    def forward(
        ...
        for group_id in group_id_list:
            if task_obj is not None:
                pooled_output = self.pooler_list[group_id](last_hidden_state) 
                logits = task_obj.train_forward(
                    ...
                    hyp_mask, 
                    decoder_opt, 
                    self.dropout_list[group_id][task_id], 
                    self.scoring_list[group_id][task_id],
                )
                return_list.append(logits)
            elif task_type in [TaskType.Span, TaskType.SpanYN]:
                assert decoder_opt != 1
                last_hidden_state = self.dropout_list[group_id][task_id](last_hidden_state)
                logits = self.scoring_list[group_id][task_id](last_hidden_state)
                start_scores, end_scores = logits.split(1, dim=-1)
                start_scores = start_scores.squeeze(-1)
                end_scores = end_scores.squeeze(-1)
                return_list.append((start_scores, end_scores))
            elif task_type == TaskType.SeqenceLabeling:
                pooled_output = last_hidden_state
                pooled_output = self.dropout_list[group_id][task_id](pooled_output)
                pooled_output = pooled_output.contiguous().view(-1, pooled_output.size(2))
                logits = self.scoring_list[group_id][task_id](pooled_output)
                return_list.append(logits)
            elif task_type == TaskType.MaskLM:
                last_hidden_state = self.dropout_list[group_id][task_id](last_hidden_state)
                logits = self.scoring_list[group_id][task_id](last_hidden_state)
                ...
                if decoder_opt == 1:
                    ...
                    logits = self.scoring_list[group_id][task_id](
