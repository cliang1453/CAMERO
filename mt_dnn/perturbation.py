...
def generate_noise(embed, mask, epsilon=1e-5):
    ''' copied from https://github.com/namisan/mt-dnn/blob/master/mt_dnn/perturbation.py'''
...
class SmartPerturbation():
    ...
    def get_adv_perturbed_logits(
        ...
        # init delta
        embed = model(*vat_args, group_id=group_id)
        noise = generate_noise(embed, attention_mask, epsilon=self.noise_var)
        ...
        for step in range(0, self.K):
            ...
            adv_logits = model(*vat_args, group_id=group_id)
            if task_type == TaskType.Regression:
                adv_loss = F.mse_loss(adv_logits, logits.detach(), reduction="sum")
            else:
                if task_type == TaskType.Ranking:
    ...
    def get_random_perturbed_embed(
        self,
        model,
        input_ids,
        token_type_ids,
